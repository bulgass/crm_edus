...
const InProgressTab = () => {
  ...
  useEffect(() => {
    const fetchClients = async () => {
      try {
        setLoading(true);
        const inProgressCollectionRef = collection(db, 'clients/GreenCard/InProgress');
  ...
  const moveToDone = async (client) => {
    try {
      const inProgressDocRef = doc(db, 'clients/GreenCard/InProgress', client.id);
      const doneDocRef = doc(db, 'clients/GreenCard/Done', client.id);
